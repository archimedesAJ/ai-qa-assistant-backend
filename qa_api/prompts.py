...
def build_requirement_text(user_story: str, acceptance_criteria: str, feature_description: str, doc_text: str, app_context: str):
    parts = []
    if app_context:
    ...
    if doc_text:
        parts.append(f"Document Extracts:\n{doc_text.strip()[:50000]}")  # trim large docs
    return "\n\n".join(parts)

def test_case_prompt(app_context: str, requirement_text: str, max_cases: int):
    return f"""
You are a senior QA engineer. Use the application context and requirements to produce high-quality, de-duplicated test cases with good coverage.
...
def test_plan_prompt(app_context: str, requirement_text: str):
    return f"""
